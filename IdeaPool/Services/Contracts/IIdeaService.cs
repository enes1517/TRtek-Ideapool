...
namespace Services.Contracts
{
    public interface IIdeaService
    {
        ...
        Task<(List<IdeaResponseDto> ideas, MetaData metaData)> GetFilteredIdeasAsync(
     string? title, IdeaCategory? category, DateTime? startDate, int? userId,IdeaParameters ideaParameters);
        ...
        Task ToggleFavoriteAsync(int userId, int ideaId);
        Task<List<IdeaResponseDto>> GetFavoriteIdeasAsync(int userId);
    }
